...
buildByAnsible: setup
	...
	sh -c 'ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook --vault-password-file .vault-pass.txt -v site.yml --tags "build"'

deployByAnsible: setup
	docker build --pull=true -t $(DOCKER_TAG)_ap_ansible_license -f Dockerfile.ansible .
	docker run --rm -t \
	--name $(DOCKER_TAG)_ap_ansible_license --rm $(DOCKER_TAG)_ap_ansible_license \
	sh -c 'ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook --vault-password-file .vault-pass.txt -v site.yml --tags "deploy"'
